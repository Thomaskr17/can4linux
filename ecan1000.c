/* ECAN1000 board specific functions
 * 
 */
...
int CAN_VendorInit (int minor)
{
   ...
   if ((Base[minor] >= 0x200) && 
       ...
       if (request_region(Base[minor], can_range[minor], "CAN-IO") == NULL) {
           DBGprint(DBG_BRANCH, ("Requested I/O: %d", Base[minor]));
       } else {
           DBGout(); return -EBUSY;
       }
   } else {
       DBGprint(DBG_BRANCH, ("Invalid I/O: 0x%x", Base[minor]));
