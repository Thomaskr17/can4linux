...
/* Index memory adress
 * Two memory adresses are defined
 * first the first address has to be written with the adress of
...
#define CANout(bd,adr,v)	do {\
        ...
        writeb(v, (void __iomem *)can_base[bd] + 1); \
  } while(0)
...
#define CANset(bd,adr,m)  {\
        ...
        writeb((u32) &regbase->adr, (void __iomem *)can_base[bd]); \
        writeb(val | m, (void __iomem *)can_base[bd] + 1); \
	}
