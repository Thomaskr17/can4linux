...
* (c) 2001-2013 Heinz-Juergen Oertel (oe@port.de)
*------------------------------------------------------------------
...
*/
    ...
    */
    ...
    irqmask =  	+ CAN_OVERRUN_INT_ENABLE
    ...
    CANset(minor, canirq_enable, irqmask);
...
Take care of The MSG_CANFD Flag and treat messages and dlc/length code different.
The SJA doesn't know anything about longer date then 8 byte,
...
u8 tx2reg, stat;
    ...
    /* with canfd we get the length information which can be larger than 4 bit
       take care
     */
    ...
    /* SJA1000 is calssic CAN with dlc instead number of bytes.
      ...
      */
    tx2reg = tx->length & 0x0f;         /* limit length in DLC to 4 bit */
    if( tx2reg > 8) { tx2reg = 8; } /* limit CAN message length to 8 */
...
/* ---------- / fill frame data -------------------------------- */
    ...
    // printk("ISR[%d] dlc= %d flags= 0x%03x\n", minor, last_Tx_object[minor].length, last_Tx_object[minor].flags);
		...
		memcpy(  
		    ...
		    (void *)&last_Tx_object[minor],
		    sizeof(canmsg_t));
		/* correct .length fill to next fitting CAM FD frame length */
		RxFifo->data[RxFifo->head].length =
			adjustlength(RxFifo->data[RxFifo->head].length);
...
Tx_done:
    ...
    */ 
    ...
    ecc = CANin(minor, errorcode);

#if 0
	//printk("CAN[%d]: error interrupt! ecc 0x%0X\n", minor, ecc);
	printk("CAN error: %s, code 0x%x, seg 0x%x\n  ",
		    (ecc & (1<<5)) ? "RX" : "TX",
		    ecc >> 6, 
		    ecc & 0x1f);
	switch(ecc >> 6) {
	...
	switch(ecc & CAN_ECC_SEGMENT_MASK) {
		...
		break;
	    case 2:
		printk("id.28 to id.21\n");
		break;
	    case 6:
		printk("id.20 to id.18\n");
		break;
	    case 4:
		printk("bit SRTR\n");
		break;
		...
		break;
	    case 7:
		printk("id.17 to id.13\n");
		break;
	    case 0x0f:
		printk("id.12 to id.5\n");
		break;
	    case 0x0e:
		printk("id.4 to id.0\n");
		break;
		...
		break;
	    case 0x0d:
		printk("reserverd bit 1\n");
		break;
	    case 9:
		printk("reserverd bit 0\n");
		break;
	    case 0x0b:
		printk("data length code\n");
		break;
	    case 0x0a:
		printk("data field\n");
		break;
		...
		break;
	    case 0x18:
		printk("crc delimiter\n");
		break;
	    case 0x19:
		printk("ack slot\n");
		break;
	    case 0x1b:
		printk("ack delimiter\n");
		break;
	    case 0x1a:
		printk("end of frame\n");
		break;
		...
		break;
	    case 0x11:
		printk("active error flag\n");
		break;
	    case 0x16:
		printk("passiver error flag\n");
		break;
	    case 0x13:
		printk("tolerate dominant bits\n");
		break;
	    case 0x17:
		printk("error delimiter\n");
		break;
	    case 0x1c:
		printk("overload flag\n");
		break;
	    default:
		printk("??\n");
		break;
	...
	(ecc & CAN_ECC_DIRECTION_MASK) ? RxErr[minor]++ : TxErr[minor]++;
